...
export default function TeamManager() {
  ...
  function showNotif(msg, color = C.green) { setNotif({ msg, color }); setTimeout(() => setNotif(null), 3000); }
  ...
  useEffect(() => {
    ...
    return onSnapshot(doc(db, "teams", profile.teamId), snap => {
      if (snap.exists()) {
        ...
        // Load managers' profiles
        if (data.managers?.length) {
          Promise.all(data.managers.map(uid => getDoc(doc(db, "users", uid)))).then(snaps => {
            setManagers(snaps.filter(s => s.exists()).map(s => ({ uid: s.id, ...s.data() })));
          });
        }
      }
      setLoading(false);
  ...
  async function createTeam() {
    ...
    // Check uniqueness
    const q = query(collection(db, "teams"), where("nameLower", "==", teamName.trim().toLowerCase()));
    const existing = await getDocs(q);
    if (!existing.empty) return showNotif("Ya existe un equipo con ese nombre", C.red);
    setSaving(true);
    let logoUrl = null;
    if (logoFile) {
  ...
  async function saveTeam() {
    ...
    setLogoFile(null); setEditing(false); setSaving(false);
  ...
  async function addManager() {
    ...
    if (snap.empty) { showNotif("No se encontró ningún usuario con ese email", C.red); setAddingManager(false); return; }
    const newManagerUid = snap.docs[0].id;
    if (myTeam.managers?.includes(newManagerUid)) { showNotif("Ese usuario ya es gestor", C.red); setAddingManager(false); return; }
    // Check if new manager already has a team
    const newManagerData = snap.docs[0].data();
    if (newManagerData.teamId) { showNotif("Ese usuario ya gestiona otro equipo", C.red); setAddingManager(false); return; }
    const newManagers = [...(myTeam.managers || []), newManagerUid];
    await updateDoc(doc(db, "teams", myTeam.id), { managers: newManagers });
    await updateDoc(doc(db, "users", newManagerUid), { teamId: myTeam.id });
    setManagerEmail(""); setAddingManager(false);
  ...
  async function removeManager(uid) {
    ...
    const newManagers = myTeam.managers.filter(m => m !== uid);
    await updateDoc(doc(db, "teams", myTeam.id), { managers: newManagers });
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            <div>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
                {logoPreview ? <img src={logoPreview} alt="preview" style={{ width: 64, height: 64, borderRadius: "50%", objectFit: "cover", border: "2px solid rgba(232,184,75,0.4)" }} /> : <div style={{ width: 64, height: 64, borderRadius: "50%", border: "2px dashed rgba(232,184,75,0.3)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 28, background: "rgba(232,184,75,0.04)", flexShrink: 0 }}>🛡</div>}
            ...
            <div><label style={S.label}>Nombre del equipo *</label><input style={S.input} placeholder="Nombre único para tu equipo" value={teamName} onChange={e => setTeamName(e.target.value)} /></div>
          ...
          <div style={S.card}>
            ...
            {managers.map(m => (
              <div key={m.uid} style={{ display: "flex", alignItems: "center", gap: 12, padding: "8px 0", borderBottom: "1px solid rgba(255,255,255,0.04)" }}>
                ...
                {m.uid === user.uid ? <span style={S.tag(C.gold)}>Tú</span> : (
            ...
            <div style={{ marginTop: 14 }}>
              ...
              <div style={{ display: "flex", gap: 8 }}>
                <input style={{ ...S.input, flex: 1 }} type="email" placeholder="email@ejemplo.com" value={managerEmail} onChange={e => setManagerEmail(e.target.value)} onKeyDown={e => e.key === "Enter" && addManager()} />
              ...
              <p style={{ margin: "6px 0 0", fontSize: 11, color: C.faint }}>El usuario debe estar registrado en la plataforma. Solo puede gestionar un equipo a la vez.</p>
            </div>
          </div>
        </div>
      )}
