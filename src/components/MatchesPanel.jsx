...
export default function MatchesPanel({ tournaments, inscriptions, currentUser, isAdmin, logoMap, myTeamNames: myTeamNamesProp }) {
  ...
  async function submitReport(day, match) {
    ...
    await saveMatchUpdate(day, match, { ...(side === "A" ? { reportByA: report } : { reportByB: report }), matchStatus: newStatus, ...(newStatus === "validado" ? { scoreA: sA, scoreB: sB, played: true, winner: sA > sB ? match.teamA : sA < sB ? match.teamB : null } : {}) });
  ...
  async function adminValidate(day, match, forceSA, forceSB) {
    ...
    await saveMatchUpdate(day, match, { reportByA: report, reportByB: report, matchStatus: "validado", scoreA: sA, scoreB: sB, played: true, winner: sA > sB ? match.teamA : sA < sB ? match.teamB : null });
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.85)", zIndex: 400, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
          <div style={{ ...S.card, maxWidth: 380, width: "100%", padding: 24 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 20 }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}><input type="number" min={0} max={99} placeholder="0" value={rScoreA} onChange={e => setRScoreA(e.target.value)} style={{ ...S.numInput, width: 58, padding: 12, fontSize: 22 }} /><span style={{ color: C.faint, fontSize: 20 }}>–</span><input type="number" min={0} max={99} placeholder="0" value={rScoreB} onChange={e => setRScoreB(e.target.value)} style={{ ...S.numInput, width: 58, padding: 12, fontSize: 22 }} /></div>
          ...
          <div style={{ ...S.card, maxWidth: 380, width: "100%", padding: 24 }}>
            ...
            {(adminEdit.reportByA || adminEdit.reportByB) && (
              <div style={{ background: "rgba(255,255,255,0.03)", borderRadius: 8, padding: 12, marginBottom: 14 }}>
                ...
                {adminEdit.reportByA && <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}><p style={{ margin: 0, fontSize: 12, color: C.muted }}><strong style={{ color: C.text }}>{adminEdit.teamA}</strong>: {adminEdit.reportByA.scoreA}–{adminEdit.reportByA.scoreB}</p><button style={{ ...S.btnInline(C.green), padding: "6px 12px", fontSize: 10 }} onClick={() => adminValidate(adminEditDay, adminEdit, adminEdit.reportByA.scoreA, adminEdit.reportByA.scoreB)}>✓ Aceptar</button></div>}
                {adminEdit.reportByB && <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}><p style={{ margin: 0, fontSize: 12, color: C.muted }}><strong style={{ color: C.text }}>{adminEdit.teamB}</strong>: {adminEdit.reportByB.scoreA}–{adminEdit.reportByB.scoreB}</p><button style={{ ...S.btnInline(C.green), padding: "6px 12px", fontSize: 10 }} onClick={() => adminValidate(adminEditDay, adminEdit, adminEdit.reportByB.scoreA, adminEdit.reportByB.scoreB)}>✓ Aceptar</button></div>}
              </div>
            )}
