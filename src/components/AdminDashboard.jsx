...
import { C, S, statusColor, buildGroups, buildSeededElimination, buildEliminationRound, TeamLogo, BottomNav } from "../shared.jsx";
      ...
      <header style={S.topBar}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          {view !== "list" && tab === "torneos" && (
            <button onClick={() => setView("list")} style={{ background: "none", border: "none", color: C.muted, cursor: "pointer", fontFamily: "'Georgia',serif", fontSize: 16, padding: "0 8px 0 0" }}>←</button>
          )}
                  ...
                  : tournaments.map(t => {
                    ...
                    return (
                      <div key={t.id} style={{ ...S.card, cursor: "pointer" }} onClick={() => { setActiveTId(t.id); setView("detail"); }}>
                        ...
                        <p style={{ margin: 0, color: C.muted, fontSize: 12 }}>{t.format} · {t.legs > 1 ? "2 vueltas" : "1 vuelta"} · {tInsc.length} equipos</p>
                      </div>
                    );
                  })
                }
              ...
              return (
                <div>
                  ...
                  {approvedInsc.length > 0 && !hasGroups && !hasElim && (
                    <div style={{ ...S.card, marginBottom: 14 }}>
                      <p style={{ ...S.label, marginBottom: 12 }}>Equipos aprobados ({approvedInsc.length})</p>
                      {approvedInsc.map(i => (
                  ...
                  {/* Standings only — matches managed from Partidos tab */}
                  {hasGroups && (
                    ...
                    <div>
                      ...
                      {t.eliminationRounds.map((round, ri) => (
                        <div key={ri} style={{ marginBottom: 14 }}>
                          <p style={{ ...S.label, color: C.gold, marginBottom: 8 }}>{require ? "" : ""}{["Final", "Semifinal", "Cuartos", "Octavos"][t.eliminationRounds.length - ri - 1] || `Ronda ${ri + 1}`}</p>
                  ...
                  <div style={{ marginTop: 16, padding: "12px 14px", background: "rgba(79,142,247,0.06)", borderRadius: 10, border: "1px solid rgba(79,142,247,0.15)" }}>
                    <p style={{ margin: "0 0 8px", fontSize: 12, color: C.blue }}>ℹ Los resultados de los partidos se gestionan en la pestaña <strong>Partidos</strong></p>
