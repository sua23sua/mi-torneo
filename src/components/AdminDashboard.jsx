...
import { C, S, statusColor, getRoundName, buildGroups, applyGroupResult, buildSeededElimination, buildEliminationRound } from "../shared";
...
export default function AdminDashboard({ onLogout }) {
  ...
  useEffect(() => {
    const unsub1 = onSnapshot(query(collection(db, "tournaments"), orderBy("createdAt", "desc")), snap => setTournaments(snap.docs.map(d => ({ id: d.id, ...d.data() }))));
    const unsub2 = onSnapshot(query(collection(db, "inscriptions"), orderBy("createdAt", "desc")), snap => setInscriptions(snap.docs.map(d => ({ id: d.id, ...d.data() }))));
    const unsub3 = onSnapshot(query(collection(db, "news"), orderBy("createdAt", "desc")), snap => setNews(snap.docs.map(d => ({ id: d.id, ...d.data() }))));
  ...
  async function generateElimFromGroups(t) {
    ...
    await updateDoc(doc(db, "tournaments", t.id), {
      eliminationRounds: [{ round: 1, matches }], status: "En curso",
    });
    showNotif(`Fase eliminatoria generada ✓ (${matches.length} partidos)`);
  }
  ...
  function editNews(n) {
    setNewsForm({ title: n.title, body: n.body, category: n.category || "Noticia" });
  ...
  const tabs = ["torneos", "inscripciones", "noticias"];
  ...
  return (
    <div style={S.wrap}>
      ...
      <style>{`input:focus,select:focus,textarea:focus{outline:none;border-color:#388bff!important;}input::placeholder,textarea::placeholder{color:rgba(200,212,228,0.2)!important;}table{border-collapse:collapse;width:100%;}@media(max-width:600px){.hide-mobile{display:none!important;}}`}</style>
      ...
      <header style={S.header}>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          ...
          <span style={{ fontSize: "clamp(13px,3vw,16px)", fontWeight: 700, letterSpacing: 2, textTransform: "uppercase", color: C.blue }}>TournamentOS</span>
          <span style={S.tag(C.blue)} className="hide-mobile">Admin</span>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <span style={{ fontSize: 11, color: C.muted }} className="hide-mobile">{profile?.name || user?.email}</span>
      ...
      <main style={S.main}>
            ...
            {view === "list" && (
              <>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20, flexWrap: "wrap", gap: 12 }}>
                  <h1 style={{ fontSize: "clamp(18px,4vw,26px)", fontWeight: 700, margin: 0 }}>Torneos · FIFA Clubes Pro</h1>
                ...
                <div>
                  ...
                  {tInscriptions.length > 0 && (
                    <div style={{ ...S.card, marginBottom: 20 }}>
                      ...
                      {tInscriptions.map(i => (
                        <div key={i.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 0", borderBottom: "1px solid rgba(255,255,255,0.04)", flexWrap: "wrap", gap: 8 }}>
                  ...
                  {hasGroups && (
                    <div style={{ marginBottom: 28 }}>
                      <p style={{ fontSize: 11, letterSpacing: 3, textTransform: "uppercase", color: C.blue, marginBottom: 14 }}>
                      ...
                      {t.groups.map((g, gi) => (
                        <div key={gi} style={{ ...S.card, marginBottom: 16 }}>
                          ...
                          <div style={{ overflowX: "auto" }}>
                            <table style={{ marginBottom: 16, minWidth: 320 }}>
                          ...
                          {g.matches.map((m, mi) => {
                            ...
                            return (
                              <div key={mi} style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 0", borderBottom: "1px solid rgba(255,255,255,0.04)", flexWrap: "wrap" }}>
                                <span style={{ flex: 1, minWidth: 80, fontWeight: m.played && m.scoreA > m.scoreB ? 700 : 400, fontSize: 13 }}>{m.teamA}</span>
                                {m.played ? (
                                  <span style={{ fontWeight: 700, letterSpacing: 3, color: C.gold, fontSize: 14 }}>{m.scoreA}—{m.scoreB}</span>
                                ) : (
                                ...
                                <span style={{ flex: 1, minWidth: 80, textAlign: "right", fontWeight: m.played && m.scoreB > m.scoreA ? 700 : 400, fontSize: 13 }}>{m.teamB}</span>
                              </div>
                            );
                          })}
                        </div>
                      ...
                      {t.format === "Grupos + Eliminatoria" && groupsDone && !hasElim && (
                        <button style={{ ...S.btn(C.gold), color: "#0a0a0f" }} onClick={() => generateElimFromGroups(t)}>
                          Generar fase eliminatoria (cruces seeded) →
                        </button>
                      )}
                    </div>
                  )}
                  ...
                  {hasElim && (
                    <div>
                      <p style={{ fontSize: 11, letterSpacing: 3, textTransform: "uppercase", color: C.blue, marginBottom: 14 }}>Fase eliminatoria</p>
                          ...
                          {round.matches.map((m, mi) => {
                            ...
                            return (
                              <div key={mi} style={{ ...S.card, display: "flex", alignItems: "center", gap: 10, padding: "12px 16px", flexWrap: "wrap", marginBottom: 8 }}>
                                <span style={{ flex: 1, minWidth: 80, fontWeight: m.winner === m.teamA ? 700 : 400, color: m.winner === m.teamA ? C.blue : C.text, fontSize: 13 }}>{m.teamA}</span>
                                {m.winner ? (
                                ...
                                <span style={{ flex: 1, minWidth: 80, textAlign: "right", fontWeight: m.winner === m.teamB ? 700 : 400, color: m.winner === m.teamB ? C.blue : C.text, fontSize: 13 }}>{m.teamB}</span>
                              </div>
                            );
                          })}
                        </div>
        ...
        {mainTab === "inscripciones" && (
          <>
            <h1 style={{ fontSize: "clamp(18px,4vw,24px)", fontWeight: 700, margin: "0 0 20px" }}>Inscripciones</h1>
            {inscriptions.length === 0 ? (
              ...
              return (
                <div key={i.id} style={{ ...S.card, display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 10 }}>
                  <div>
                    <p style={{ margin: "0 0 4px", fontWeight: 600, fontSize: 13 }}>{i.userName} — <span style={{ color: C.muted, fontWeight: 400 }}>{i.teamName}</span></p>
        ...
        {mainTab === "noticias" && (
          <>
            <h1 style={{ fontSize: "clamp(18px,4vw,24px)", fontWeight: 700, margin: "0 0 20px" }}>Gestión de Noticias</h1>
            ...
            {/* List */}
            {news.length === 0 ? (
              <div style={{ border: "1px dashed rgba(255,255,255,0.08)", padding: 32, textAlign: "center", color: C.faint }}>No hay noticias publicadas.</div>
            ) : news.map(n => (
              <div key={n.id} style={S.card}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "start", flexWrap: "wrap", gap: 10, marginBottom: 8 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                    <span style={S.tag(n.category === "Aviso" ? C.red : n.category === "Resultado" ? C.green : C.blue)}>{n.category}</span>
                    <h3 style={{ margin: 0, fontSize: 15 }}>{n.title}</h3>
                  </div>
                  <div style={{ display: "flex", gap: 8 }}>
                    <button style={S.btnSm} onClick={() => editNews(n)}>Editar</button>
                    <button style={S.btnDanger} onClick={() => deleteNews(n.id)}>Eliminar</button>
                  </div>
                </div>
                <p style={{ margin: 0, color: C.muted, fontSize: 12 }}>{new Date(n.createdAt).toLocaleDateString("es-ES")}</p>
                <p style={{ margin: "8px 0 0", fontSize: 13, color: "#9aa8bc", lineHeight: 1.6, whiteSpace: "pre-wrap" }}>{n.body.substring(0, 200)}{n.body.length > 200 ? "..." : ""}</p>
              </div>
            ))}
          </>
        )}
