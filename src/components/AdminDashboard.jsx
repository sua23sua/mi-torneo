...
import { C, S, statusColor, buildGroups, buildSeededElimination, buildEliminationRound, buildSwissGroup, buildSwissCups, buildDivision, applyGroupResult, TeamLogo, ResponsiveNav, formatDatetime, getRoundName, isTournamentActive, TOURNAMENT_TYPES, useIsMobile } from "../shared.jsx";
...
export default function AdminDashboard({ onLogout }) {
  ...
  const [divisionAssign, setDivisionAssign] = useState({});
  ...
  useEffect(() => {
    ...
    if (view === "divassign" && activeTId) {
      ...
      if (t) {
        ...
        const init = {};
        approved.forEach(i => { init[i.teamName] = t.divisionAssignments?.[i.teamName] ?? 0; });
  ...
  async function startTournament(t) {
    ...
    if (t.format === "Suizo") {
      await updateDoc(doc(db, "tournaments", t.id), { teams: teamNames, groups: [buildSwissGroup(teamNames, t.swissRounds || 5)], swissCups: [], status: "En curso" });
      ...
      teamNames.forEach(name => { const idx = assignments[name] ?? 0; divTeams[Math.min(idx, divCount - 1)].push(name); });
      const divisions = divTeams.map((teams, i) => teams.length < 2 ? { name: `División ${i + 1}`, teams, groups: [], legs: t.legs || 1 } : buildDivision({ name: `División ${i + 1}`, teams, groupCount: t.groupsPerDivision || 1 }, t.legs || 1));
  ...
  async function generateSwissCups(t) {
    ...
    if (!group.matches.every(m => m.matchStatus === "validado")) return showNotif("Aún hay partidos pendientes");
  ...
  async function resetTournament(t) {
    if (!window.confirm(`¿Reiniciar "${t.name}"?`)) return;
    setResetting(true);
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
        {showNameDesc && (
          <>
            ...
            <div><label style={S.label}>Enlace WhatsApp</label><input style={S.input} placeholder="https://chat.whatsapp.com/..." value={values.whatsappLink} onChange={e => onChange("whatsappLink", e.target.value)} /></div>
          </>
        )}
        <div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {Object.values(TOURNAMENT_TYPES).map(tt => (
              <button key={tt.id} onClick={() => onChange("tournamentType", tt.id)} style={{ padding: "14px 16px", borderRadius: 10, border: `1px solid ${values.tournamentType === tt.id ? tt.color : "rgba(255,255,255,0.09)"}`, background: values.tournamentType === tt.id ? `${tt.color}14` : "rgba(255,255,255,0.02)", cursor: "pointer", fontFamily: "'Georgia',serif", textAlign: "left", display: "flex", alignItems: "center", gap: 12 }}>
                ...
                <p style={{ margin: 0, fontSize: 13, fontWeight: 700, color: values.tournamentType === tt.id ? tt.color : C.muted }}>K={tt.kBase}</p>
              </button>
            ))}
          </div>
        </div>
        ...
        {format === "Suizo" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            <div style={{ ...S.card, background: "rgba(79,142,247,0.04)", border: "1px solid rgba(79,142,247,0.15)" }}><p style={{ margin: "0 0 4px", fontSize: 12, color: C.blue, fontWeight: 700 }}>🔄 Torneo Suizo</p><p style={{ margin: 0, fontSize: 12, color: C.muted }}>Grupo único · X jornadas · Copa Oro, Plata y Bronce</p></div>
        ...
        {format === "Liga Divisiones" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            <div style={{ ...S.card, background: "rgba(167,139,250,0.04)", border: "1px solid rgba(167,139,250,0.15)" }}><p style={{ margin: "0 0 4px", fontSize: 12, color: C.purple, fontWeight: 700 }}>🏛 Liga Divisiones</p><p style={{ margin: 0, fontSize: 12, color: C.muted }}>Cada división es independiente. El admin asigna los equipos.</p></div>
  ...
  function TournamentCard({ t }) {
    ...
    const tConflicts = [...(t.groups || []).flatMap(g => g.matches || []), ...(t.eliminationRounds || []).flatMap(r => r.matches || []), ...(t.swissCups || []).flatMap(c => c.rounds?.flatMap(r => r.matches || []) || []), ...(t.divisions || []).flatMap(d => d.groups?.flatMap(g => g.matches || []) || [])].filter(m => m.matchStatus === "conflicto").length;
      ...
      <div style={{ ...S.card, cursor: "pointer" }} onClick={() => { setActiveTId(t.id); setView("detail"); }}>
        <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 10 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            <div><span style={{ fontSize: 10, color: ttype.color, letterSpacing: 1, textTransform: "uppercase" }}>{ttype.label}</span><span style={{ fontSize: 10, color: C.faint, marginLeft: 6 }}>{t.format}</span></div>
          </div>
    ...
    return (
      <div>
        <div style={{ marginBottom: 12 }}>
          ...
          <p style={{ margin: 0, color: C.muted, fontSize: 12 }}>{t.format} · {approvedInsc.length} equipos · <span style={{ color: ttype.color }}>{ttype.label} (K={ttype.kBase})</span></p>
        </div>
        ...
        {t.format === "Suizo" && hasGroups && swissGroupDone && !hasSwissCups && <div style={{ background: "rgba(232,184,75,0.08)", border: "1px solid rgba(232,184,75,0.25)", borderRadius: 10, padding: 16, marginBottom: 14 }}><p style={{ margin: "0 0 10px", color: C.gold, fontSize: 13 }}>✓ Fase de grupos completada</p><button style={S.btn(C.gold)} onClick={() => generateSwissCups(t)}>🏆 Generar Copas →</button></div>}
          ...
          <div style={{ ...S.card, marginBottom: 14 }}>
            ...
            {pendingInsc.map(i => (
              <div key={i.id} style={{ paddingBottom: 12, marginBottom: 12, borderBottom: "1px solid rgba(255,255,255,0.04)" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 10 }}><TeamLogo name={i.teamName} logoUrl={i.logoUrl} size={40} /><div><p style={{ margin: "0 0 2px", fontWeight: 700, fontSize: 14 }}>{i.teamName}</p><p style={{ margin: 0, color: C.muted, fontSize: 12 }}>{i.userName}</p></div></div>
                <div style={{ display: "flex", gap: 8 }}><button style={{ ...S.btnSm, flex: 1, borderColor: "rgba(82,214,138,0.4)", color: C.green }} onClick={() => handleInscription(i.id, "aprobada")}>Aprobar</button><button style={{ ...S.btnDanger, flex: 1 }} onClick={() => handleInscription(i.id, "rechazada")}>Rechazar</button></div>
              </div>
            ))}
          </div>
        )}
        ...
        {approvedInsc.length > 0 && !hasStarted && (
          <div style={{ ...S.card, marginBottom: 14 }}>
            ...
            {approvedInsc.map(i => <div key={i.id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 0", borderBottom: "1px solid rgba(255,255,255,0.03)" }}><TeamLogo name={i.teamName} logoUrl={i.logoUrl} size={32} /><div><p style={{ margin: "0 0 1px", fontWeight: 600, fontSize: 13 }}>{i.teamName}</p>{t.format === "Liga Divisiones" && t.divisionAssignments?.[i.teamName] !== undefined && <p style={{ margin: 0, fontSize: 11, color: C.purple }}>División {(t.divisionAssignments[i.teamName] || 0) + 1}</p>}</div></div>)}
          </div>
        )}
          ...
          <div style={{ marginBottom: 16 }}>
            ...
            <div style={S.card}>
              <div style={{ overflowX: "auto" }}>
                <table><thead><tr>{["#", "Equipo", "PJ", "PTS", "GF", "GC", "DG"].map(h => <th key={h} style={S.th}>{h}</th>)}</tr></thead>
                  <tbody>{(t.groups[0]?.standings || []).map((s, si) => <tr key={s.name} style={{ background: si < (t.swissCupSize || 4) ? "rgba(232,184,75,0.06)" : si < (t.swissCupSize || 4) * 2 ? "rgba(148,163,184,0.04)" : "transparent" }}><td style={{ ...S.td, color: C.faint }}>{si + 1}</td><td style={S.td}><TRow name={s.name} size={18} /></td><td style={{ ...S.td, textAlign: "center" }}>{s.pj}</td><td style={{ ...S.td, fontWeight: 700, textAlign: "center" }}>{s.pts}</td><td style={{ ...S.td, textAlign: "center" }}>{s.gf}</td><td style={{ ...S.td, textAlign: "center" }}>{s.gc}</td><td style={{ ...S.td, color: s.gd >= 0 ? C.green : C.red, textAlign: "center" }}>{s.gd > 0 ? "+" : ""}{s.gd}</td></tr>)}</tbody>
                </table>
              </div>
            </div>
          </div>
          ...
          <div style={{ marginBottom: 16 }}>
            ...
            {t.swissCups.map((cup, ci) => (
              <div key={ci} style={{ ...S.card, borderColor: `${CUP_COLORS[cup.name] || C.muted}30`, marginBottom: 10 }}>
                <p style={{ margin: "0 0 12px", fontWeight: 700, fontSize: 15, color: CUP_COLORS[cup.name] || C.muted }}>{cup.name === "Oro" ? "🥇" : cup.name === "Plata" ? "🥈" : cup.name === "Bronce" ? "🥉" : "🏅"} Copa {cup.name} <span style={{ fontSize: 11, color: C.faint, fontWeight: 400 }}>({cup.startPos}º–{cup.endPos}º)</span></p>
                {cup.rounds?.map((round, ri) => <div key={ri} style={{ marginBottom: 10 }}><p style={{ ...S.label, marginBottom: 6 }}>{getRoundName(cup.rounds.length, ri)}</p>{round.matches.map((m, mi) => <div key={mi} style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 10px", background: "rgba(255,255,255,0.02)", borderRadius: 8, marginBottom: 5 }}><div style={{ flex: 1, display: "flex", alignItems: "center", gap: 6, minWidth: 0 }}><TeamLogo name={m.teamA} logoUrl={logoMap[m.teamA]} size={20} /><span style={{ fontSize: 13, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{m.teamA}</span></div><div style={{ minWidth: 50, textAlign: "center" }}>{m.matchStatus === "validado" ? <span style={{ fontWeight: 700, color: CUP_COLORS[cup.name] || C.gold, fontSize: 14 }}>{m.scoreA}–{m.scoreB}</span> : <span style={{ color: C.faint, fontSize: 11 }}>pdte</span>}</div><div style={{ flex: 1, display: "flex", alignItems: "center", gap: 6, justifyContent: "flex-end", minWidth: 0 }}><span style={{ fontSize: 13, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{m.teamB}</span><TeamLogo name={m.teamB} logoUrl={logoMap[m.teamB]} size={20} /></div></div>)}</div>)}
              ...
              <div key={di} style={{ marginBottom: 20 }}>
                ...
                {div.groups?.map((g, gi) => (
                  <div key={gi} style={{ ...S.card, marginBottom: 8 }}>
                    ...
                    <div style={{ overflowX: "auto" }}><table><thead><tr>{["#", "Equipo", "PJ", "PTS", "GF", "GC", "DG"].map(h => <th key={h} style={S.th}>{h}</th>)}</tr></thead><tbody>{g.standings.map((s, si) => <tr key={s.name}><td style={{ ...S.td, color: C.faint }}>{si + 1}</td><td style={S.td}><TRow name={s.name} size={18} /></td><td style={{ ...S.td, textAlign: "center" }}>{s.pj}</td><td style={{ ...S.td, fontWeight: 700, textAlign: "center" }}>{s.pts}</td><td style={{ ...S.td, textAlign: "center" }}>{s.gf}</td><td style={{ ...S.td, textAlign: "center" }}>{s.gc}</td><td style={{ ...S.td, color: s.gd >= 0 ? C.green : C.red, textAlign: "center" }}>{s.gd > 0 ? "+" : ""}{s.gd}</td></tr>)}</tbody></table></div>
                  </div>
                ))}
              </div>
            ))}
        ...
        {hasGroups && !["Suizo"].includes(t.format) && (
          <div style={{ marginBottom: 16 }}>
            ...
            {t.groups.map((g, gi) => <div key={gi} style={{ ...S.card, marginBottom: 10 }}><p style={{ ...S.label, color: C.gold, marginBottom: 10 }}>Grupo {g.name}</p><div style={{ overflowX: "auto" }}><table><thead><tr>{["#", "Equipo", "PJ", "PTS", "GF", "GC", "DG"].map(h => <th key={h} style={S.th}>{h}</th>)}</tr></thead><tbody>{g.standings.map((s, si) => <tr key={s.name} style={{ background: si < (t.qualify || 2) && t.format === "Grupos + Eliminatoria" ? "rgba(79,142,247,0.06)" : "transparent" }}><td style={{ ...S.td, color: C.faint }}>{si + 1}</td><td style={S.td}><TRow name={s.name} size={18} /></td><td style={{ ...S.td, textAlign: "center" }}>{s.pj}</td><td style={{ ...S.td, fontWeight: 700, textAlign: "center" }}>{s.pts}</td><td style={{ ...S.td, textAlign: "center" }}>{s.gf}</td><td style={{ ...S.td, textAlign: "center" }}>{s.gc}</td><td style={{ ...S.td, color: s.gd >= 0 ? C.green : C.red, textAlign: "center" }}>{s.gd > 0 ? "+" : ""}{s.gd}</td></tr>)}</tbody></table></div></div>)}
            {t.format === "Grupos + Eliminatoria" && t.groups.every(g => g.matches.every(m => m.matchStatus === "validado")) && !hasElim && <button style={S.btn(C.gold)} onClick={async () => { const matches = buildSeededElimination(t.groups, t.qualify || 2); if (!matches.length) return showNotif("No hay suficientes clasificados"); await updateDoc(doc(db, "tournaments", t.id), { eliminationRounds: [{ round: 1, matches }] }); showNotif("Fase eliminatoria generada ✓"); }}>Generar fase eliminatoria →</button>}
          </div>
        )}
        ...
        {hasElim && (
          <div style={{ marginBottom: 16 }}>
            ...
            {t.eliminationRounds.map((round, ri) => <div key={ri} style={{ marginBottom: 10 }}><p style={{ ...S.label, color: C.gold, marginBottom: 6 }}>{getRoundName(t.eliminationRounds.length, ri)}</p>{round.matches.map((m, mi) => <div key={mi} style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 12px", background: "rgba(255,255,255,0.02)", borderRadius: 8, marginBottom: 5 }}><div style={{ flex: 1, display: "flex", alignItems: "center", gap: 6, minWidth: 0 }}><TeamLogo name={m.teamA} logoUrl={logoMap[m.teamA]} size={20} /><span style={{ fontSize: 13, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{m.teamA}</span></div><div style={{ minWidth: 54, textAlign: "center" }}>{m.matchStatus === "validado" ? <span style={{ fontWeight: 700, color: C.gold, fontSize: 14 }}>{m.scoreA}–{m.scoreB}</span> : <span style={{ ...S.tag(m.matchStatus === "conflicto" ? C.red : m.matchStatus === "parcial" ? C.orange : C.muted), fontSize: 8 }}>{m.matchStatus || "pdte"}</span>}</div><div style={{ flex: 1, display: "flex", alignItems: "center", gap: 6, justifyContent: "flex-end", minWidth: 0 }}><span style={{ fontSize: 13, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{m.teamB}</span><TeamLogo name={m.teamB} logoUrl={logoMap[m.teamB]} size={20} /></div></div>)}</div>)}
          </div>
        )}
  ...
  return (
    <div style={S.wrap}>
      {notif && <div style={{ position: "fixed", top: 16, left: "50%", transform: "translateX(-50%)", width: "min(90vw, 420px)", background: C.blue, color: "#fff", padding: "13px 16px", zIndex: 1000, fontSize: 13, fontFamily: "'Georgia',serif", boxShadow: "0 8px 32px rgba(0,0,0,0.5)", borderRadius: 10, textAlign: "center", fontWeight: 600 }}>{notif}</div>}
      ...
      {/* Responsive Nav — TopNav en PC, header+BottomNav en móvil */}
      <ResponsiveNav
        tabs={navTabs}
        active={tab}
        onChange={(t) => { setTab(t); if (t !== "torneos") setView("list"); }}
        color={C.blue}
        rightContent={rightContent}
      />

      {/* Mobile-only sticky header (back button + title) */}
      {isMobile && (
      ...
      {/* PC back button */}
      {!isMobile && view !== "list" && tab === "torneos" && (
        <div style={{ maxWidth: 960, margin: "0 auto", padding: "12px 32px 0" }}>
          <button onClick={handleBack} style={{ background: "none", border: "none", color: C.muted, cursor: "pointer", fontFamily: "'Georgia',serif", fontSize: 13, padding: 0 }}>← Volver</button>
        </div>
      )}
          ...
          <>
            {view === "list" && (
              <>
                ...
                {/* PC: 2-column grid for tournament cards */}
                <div style={S.grid(isMobile)}>
                  {tourneySubTab === "activos" && (
                    activeTournaments.length === 0
                      ? <div style={{ ...S.card, textAlign: "center", padding: 48, color: C.faint, gridColumn: "1/-1" }}><div style={{ fontSize: 36, marginBottom: 12 }}>🎮</div>No hay torneos activos.</div>
                ...
                {tourneySubTab === "historial" && (
                  <div>
                    ...
                    <div style={S.grid(isMobile)}>
                      {filteredHistory.length === 0 ? <div style={{ ...S.card, textAlign: "center", padding: 40, color: C.faint, gridColumn: "1/-1" }}>No hay torneos en {MONTHS[histMonth]} {histYear}.</div> : filteredHistory.map(t => <TournamentCard key={t.id} t={t} />)}
                    </div>
                  </div>
                )}
              </>
            ...
            {view === "divassign" && activeTournament && (
              <>
                ...
                <div style={{ ...S.card, background: "rgba(167,139,250,0.06)", border: "1px solid rgba(167,139,250,0.2)", marginBottom: 16 }}>
                  <p style={{ margin: 0, fontSize: 12, color: C.purple }}>Asigna cada equipo a su división. Tienes {activeTournament.divisionCount} divisiones.</p>
                </div>
                <div style={S.grid(isMobile)}>
                  {inscriptions.filter(i => i.tournamentId === activeTId && i.status === "aprobada").map(i => (
                    <div key={i.id} style={{ ...S.card, display: "flex", alignItems: "center", gap: 12, marginBottom: 8 }}>
                      ...
                      <select style={{ ...S.select, width: 140, padding: "10px 12px" }} value={divisionAssign[i.teamName] ?? 0} onChange={e => setDivisionAssign(p => ({ ...p, [i.teamName]: +e.target.value }))}>
                        {Array.from({ length: activeTournament.divisionCount || 2 }, (_, idx) => <option key={idx} value={idx}>División {idx + 1}</option>)}
                      </select>
                    </div>
                  ))}
                </div>
            ...
            {view === "schedule" && activeTournament && (
              <>
                ...
                {scheduleMatchdays.length > 0 && <button style={{ ...S.btn(), opacity: savingSchedule ? 0.6 : 1, marginTop: 8 }} onClick={saveSchedule} disabled={savingSchedule}>{savingSchedule ? "Guardando..." : "Guardar horarios →"}</button>}
              </>
            )}
