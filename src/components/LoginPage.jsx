      ...
      <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
        <div style={{ width: "100%", maxWidth: 400 }}>
          ...
          <p style={{ margin: "0 0 28px", textAlign: "center", color: "#5a6880", fontSize: 14 }}>
            {mode === "login" ? "Accede a tu panel" : "Únete a la competición"}
          </p>

          {error && (
            <div style={{ background: "rgba(247,111,111,0.08)", border: "1px solid rgba(247,111,111,0.25)", color: "#f76f6f", padding: "13px 14px", fontSize: 14, marginBottom: 18, borderRadius: 10 }}>
              {error}
            </div>
          )}
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            ...
            <div>
              ...
              <input type="email" style={inputStyle} placeholder="tu@email.com" value={email} onChange={e => setEmail(e.target.value)} onKeyDown={e => e.key === "Enter" && handleSubmit()} />
            </div>
            <div>
          ...
          <div style={{ textAlign: "center", marginTop: 24, fontSize: 14, color: "#5a6880" }}>
            {mode === "login"
              ? <>¿No tienes cuenta? <span style={{ color: "#4f8ef7", cursor: "pointer" }} onClick={() => { setMode("register"); setError(""); }}>Regístrate</span></>
              : <>¿Ya tienes cuenta? <span style={{ color: "#4f8ef7", cursor: "pointer" }} onClick={() => { setMode("login"); setError(""); }}>Inicia sesión</span></>}
          </div>
        </div>
      </div>
    </div>
